...
from senteval.binary import CREval, MREval, MPQAEval, SUBJEval, SQuADBinEval
from senteval.snli import SNLIEval
from senteval.trec import TRECEval
from senteval.sick import SICKRelatednessEval, SICKEntailmentEval
from senteval.mrpc import MRPCEval
from senteval.sts import STS12Eval, STS13Eval, STS14Eval, STS15Eval, \
        STS16Eval, STSBenchmarkEval
from senteval.squad_rank import SQuADEval
from senteval.sst import SSTBinaryEval
from senteval.rank import ImageCaptionRetrievalEval
from senteval.reasoning import ReasoningEval
from senteval.quora import QuoraEval
from senteval.rte8 import RTE8Eval
...
class SentEval(object):
    def __init__(self, params, batcher, prepare=None):
        ...
        self.list_tasks = ['CR', 'MR', 'MPQA', 'SUBJ', 'SST', 'TREC', 'MRPC',
                           ...
                           'STS14', 'STS15', 'STS16', 'SQuAD', 'Reasoning',
                           'Quora', 'RTE8']

    def eval(self, name, small=0, lower=0):
        # evaluate on evaluation [name], either takes string or list of strings
        if (isinstance(name, list)):
            self.results = {x: self.eval(x, small=small, lower=lower) 
                            for x in name}
            return self.results
        ...
        if name == 'CR':
            ...
            self.evaluation = SQuADEval(tpath + '/SQuAD', 
                                       seed=self.params.seed, small=small)
        elif name == 'Reasoning':
            self.evaluation = ReasoningEval(tpath + '/argument-reasoning',
                                        seed=self.params.seed, lower=lower)
        elif name == 'Quora':
            self.evaluation = QuoraEval(tpath + '/Quora',
                                       seed=self.params.seed, small=small,
                                       lower=lower)
        elif name == 'RTE8':
            self.evaluation = RTE8Eval(tpath + '/rte8/semeval2013-Task7-2and3way',
                                        seed=self.params.seed, lower=lower,
                                        granularity=2)
