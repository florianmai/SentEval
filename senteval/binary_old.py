...
class SQuADBinEval(BinaryClassifierEval):
    def __init__(self, task_path, seed=1111):
        logging.debug('***** Transfer task : SQuAD Binary *****\n\n')
        pos = self.loadFile(os.path.join(task_path, 'squad_ranking.pos'))
        neg = self.loadFile(os.path.join(task_path, 'squad_ranking.neg'))
        super(self.__class__, self).__init__(pos, neg, seed)
